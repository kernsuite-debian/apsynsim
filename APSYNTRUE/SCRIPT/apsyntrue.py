...
# draw lat/lon grid lines every 30 degrees.
  ...
  #  self.fmtA3 = '\n%6.1fm | %6.1fm'
    ...
    self.dirtyPlot.set_title('DIRTY IMAGE',rotation=-90.,position=(1.05,0.75),transform=self.dirtyPlot.transAxes,fontsize=fs)
    self.UVPlot.set_title('UV SPACE',rotation=-90.,position=(1.05,0.75),transform=self.UVPlot.transAxes,fontsize=fs)
    self.dataFig.set_title('VISIBILITIES',rotation=0.,position=(0.1,1.05),transform=self.dataFig.transAxes,fontsize=fs)
...
##
#               if len(np.shape(FQ.data['IF FREQ']))==1:
#                 self.freqs = self.Nu0 + FQ.data['IF FREQ']
#               else:
#                 self.freqs = self.Nu0 + FQ.data['IF FREQ'][0]
#  We rather average over IFs:
               self.freqs = [self.Nu0 + np.average(FQ.data['IF FREQ'])]

           
#               self.NIF = len(self.freqs) 
...
## Figure out if there is a sign flip in the antenna coordinates:
   ...
   ## Get Hour Angle (UT corrected by GST) and Baseline:
    ...
    B0 = (self.antcoords[self.ant2[i0]-1,:] - self.antcoords[self.ant1[i0]-1,:])/self.c
   # Compute U and V:
...
####################
  ...
  #  print 'Geom: ',B0,H0S,D0S, GST0, self.RA     
    ...
    self.Mask = np.zeros(self.Nvis,dtype=np.bool)
    self.flags = np.zeros(self.Nvis,dtype=np.bool)

    self.UV = np.array(self.UV,dtype=np.float)
    ...
    self.JDate = np.array(self.JDate,dtype=np.float)
    self.ant1 = np.array(self.ant1,dtype=np.int)
    self.ant2 = np.array(self.ant2,dtype=np.int)
    self.vis = np.array(self.vis,dtype=np.complex128)
    self.wgt = np.array(self.wgt,dtype=np.float)
...
class CLEANer(object):
  ...
  def __init__(self,parent):
    ...
    self.mask = np.zeros(np.shape(self.parent.Dirty),dtype=np.bool)
    self.bmask = np.zeros(np.shape(self.parent.Dirty),dtype=np.bool)
  ...
  def _onRelease(self,event):
    ...
    if self.moved:
      ...
      self.MaskPlot = self.ResidPlot.contour(np.linspace(self.Xmax,-self.Xmax,self.parent.Npix/2),np.linspace(self.Xmax,-self.Xmax,self.parent.Npix/2),self.mask[self.X0:self.X1, self.X0:self.X1],levels=[0.5])
  ...
  def _CLEAN(self):
     ...
     for i in range(niter):
       ...
       self.residuals -= gain*peakval*np.roll(np.roll(self.PSF,peakpos[0]-self.parent.Npix/2,axis=0), peakpos[1]-self.parent.Npix/2,axis=1)
...
class MEMer(object):
  ...
  def __init__(self,parent):
    ...
    self.ResidPlot.set_adjustable('box-forced')
    self.MEMPlot.set_adjustable('box-forced')
